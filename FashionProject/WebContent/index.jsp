			...
			<c:forEach var="item" items="${productList}">
				<div class="c6366">
					<div class="c6367">
						...
						<div class="c6369">
							<p><fmt:formatNumber value="${item.price}" maxFractionDigits="0"></fmt:formatNumber> VNĐ</p>
						</div>
						<div class="c6190">
							<a href = "javascript:void(0)" onclick="addCart(${item.id})">Thêm giỏ hàng</a>
						</div>
					</div>
				</div>
			</c:forEach>
			...
			<div class="c6385">
				<ul class="c3590 c5568">
					<c:if test="${currentPage != 1 }">
						<li><a href="home.html?page=${currentPage - 1}">«</a></li>
					</c:if>
					<c:forEach var="number" begin="1" end="${noOfPages}">
						<c:choose>
							<c:when test="${currentPage == number}">
								<li class="active"><a href="home.html?page=${number}">${number}</a></li>
							</c:when>
							<c:otherwise>
								<li><a href="home.html?page=${number}">${number}</a></li>
							</c:otherwise>
						</c:choose>
					</c:forEach>
